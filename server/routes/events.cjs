...
router.get('/', async (req, res) => {
  try {
    ...
    res.json(rows);
  } catch (err) { sendError(res, err, 'GET /events'); }
});
...
router.post('/', requireAdmin, async (req, res) => {
  ...
  } catch (err) { sendError(res, err, 'POST /events'); }
});
